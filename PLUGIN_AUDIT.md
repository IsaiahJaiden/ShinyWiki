...
| `/spawn` ownership | Collision between EssentialsXSpawn and a DeluxeMenus menu (`register_command: true`). Player-visible behavior is equivalent either way, so documented generically. |
| ~~`/afk` ownership~~ | **RESOLVED (owner-confirmed, 2026-08-21):** DeluxeMenus' `afk.yml` wins, not EssentialsX. In-game, `/afk` opens a GUI with a teleport-to-AFK-zone option — it does **not** set an away-from-keyboard status the way `essentials.afk`'s grant in LuckPerms had suggested. Static analysis alone got this wrong; corrected from direct in-game testing. Wiki updated accordingly. |
